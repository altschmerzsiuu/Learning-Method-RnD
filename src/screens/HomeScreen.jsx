...
const STATUS_ICON = {
  done:   <CheckCircle2 size={16} strokeWidth={2} className="text-primary-500" />,
  ...
  return (
    <PageWrapper>
      ...
      <div className="px-4 pb-28 pt-5 flex flex-col gap-5">
        ...
        {/* Hero Card */}
        <Card className="bg-primary-500 border-primary-600 text-white p-5 batik-overlay">
          ...
          <div className="flex gap-2">
            {weekDots.map((dot, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className={`w-full h-1.5 rounded-full ${dot.isActive ? 'bg-primary-500' : 'bg-border'}`} />
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-9 h-9 rounded-md flex items-center justify-center shrink-0 font-serif font-black text-[15px] ${
                    status === 'done'   ? 'bg-primary-500 text-white' :
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <Card hoverable onClick={() => navigate('/latihan')} className="text-center py-5">
              <BookOpen size={28} strokeWidth={1.5} className="mx-auto text-primary-500 mb-2" />
            ...
            <Card hoverable onClick={() => navigate('/latihan/duel')} className="text-center py-5">
              <div className="w-7 h-7 mx-auto mb-2 grid grid-cols-2 gap-0.5">
                {[0,1,2,3].map(i => <div key={i} className={`rounded-sm ${i%2===0?'bg-primary-500':'bg-accent'}`} />)}
              </div>
              <p className="font-sans text-[13px] font-bold text-ink">Duel Eksposisi</p>
              <p className="font-sans text-[11px] text-ink-muted mt-0.5">Lawan AI</p>
            </Card>
          </div>
        </section>
